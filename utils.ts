import { DecalPrompt, HelmetType, FullViewAngle, FullViewResolution } from './types';
...
export const assembleFullViewCanvas = async (images: { front: FullViewAngle, left: FullViewAngle, right: FullViewAngle, back: FullViewAngle }, resolution: FullViewResolution): Promise<string> => {
    return new Promise((resolve) => {
        ...
        ctx.fillStyle = '#2a2a2a'; // Match panel background
        ctx.fillRect(0, 0, resolution, resolution);

        const margin = resolution * 0.03125; // 64px on 2048
        const gap = resolution * 0.015625;    // 32px on 2048
        const cellWidth = (resolution - margin * 2 - gap) / 2;
        const cellHeight = (resolution - margin * 2 - gap) / 2;

        const positions = [
            { key: 'front' as const, x: margin, y: margin },
            { key: 'right' as const, x: margin + cellWidth + gap, y: margin },
            { key: 'left' as const, x: margin, y: margin + cellHeight + gap },
            { key: 'back' as const, x: margin + cellWidth + gap, y: margin + cellHeight + gap },
        ];

        let loadedImages = 0;
        const imageElements: { [key: string]: HTMLImageElement } = {};

        Object.values(images).forEach(({ src }, index) => {
            ...
            img.onload = () => {
                ...
                if (loadedImages === 4) {
                    positions.forEach(pos => {
                        const imgToDraw = imageElements[pos.key];
                        const viewData = images[pos.key];
                        if (!imgToDraw || !viewData) return;

                        const hRatio = cellWidth / imgToDraw.width;
                        const vRatio = cellHeight / imgToDraw.height;
                        const ratio = Math.min(hRatio, vRatio) * 0.95; 
                        const centerShiftX = (cellWidth - imgToDraw.width * ratio) / 2;
                        const centerShiftY = (cellHeight - imgToDraw.height * ratio) / 2;
                        ...
                        ctx.drawImage(imgToDraw, 0, 0, imgToDraw.width, imgToDraw.height,
                            pos.x + centerShiftX, pos.y + centerShiftY, imgToDraw.width * ratio, imgToDraw.height * ratio);
                        
                        ctx.restore();
                        
                        // Draw angle chip
                        const fontSize = resolution * 0.01; // 20px on 2048
                        ctx.font = `600 ${fontSize}px Inter, sans-serif`;
                        ctx.fillStyle = 'rgba(0, 0, 0, 0.6)';
                        const text = `${viewData.angle}°`;
                        const textMetrics = ctx.measureText(text);
                        const textWidth = textMetrics.width;
                        const chipPadding = fontSize * 0.4;
                        ctx.fillRect(pos.x + chipPadding, pos.y + chipPadding, textWidth + chipPadding*2, fontSize + chipPadding*2);
                        ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
                        ctx.fillText(text, pos.x + chipPadding*2, pos.y + chipPadding + fontSize);
                    });
